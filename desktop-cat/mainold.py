...
EVENTS = { # eventNumber: [[actionOrderToBeCompleted], [PossibleNextEventNumbers]]
    ...
    6: [[7], [0,0,1,1,4,7,7,7,7,8,9,8,9,12]], # touch
    ...
    14: [[17], [0,0,1,1,4,8,9,8,9,12,15,15,15,15]], # touch R
...
def event(cycle,event_number):
    times = [600, 1200, 1200, 1050, 900]
    if event_number in [1, 2, 4, 5] or event_number-10 in [1, 2, 4, 5]:
        window.after(200,update,cycle,event_number)
        
    elif event_number in [8, 9, 0] or event_number-10 in [8, 9, 0]:
        window.after(150,update,cycle,event_number)
        
    elif event_number in [6,16]:
        window.after(1000,update,cycle,event_number)
        
    elif event_number in [3, 13]:
        window.after(160,update,cycle,event_number)
...
def update(cycle,event_number):
    global animation_running,x
    ...
    window.after(1,event,cycle,event_number)
...
def on_drag_motion(event):
    global newx,ondrag,animation_running
    animation_running = False
    ondrag = True
    deltax = event.x_root - newx
    newx = window.winfo_x() + deltax
    newx = max(min(newx, 1600), 1200)
    window.geometry(f"+{newx}+{window.winfo_y()}")

def on_drag_stop(event):
    global animation_running,current_event_cycle,current_event_cycle_index,newx,ondrag
    animation_running = True
    if ondrag:
        x = newx
        print(x)
        print("Drag stopped at:", event.x, event.y)
        current_event_cycle = 6
        current_event_cycle_index = 7
        window.after(100,update,0,7)
...
if __name__ == '__main__':
    ...
    label.pack()
    #loop the program
    window.after(1,update,cycle,event_number)
    window.mainloop()
